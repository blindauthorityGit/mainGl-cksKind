...
const Element = ({ data, i, isWorkshop, isDetail, link, partner }) => {
    ...
    return (
        <motion.div
            ref={ref}
            className={`${partner ? "col-span-6" : "col-span-12"} md:col-span-6 lg:col-span-4 `}
            initial="hidden"
            animate={controls}
            variants={variants}
        >
            <Link href={`${link ? link : "#"}`}>
                {partner ? (
                    <ContainImage
                        ...
                        className="w-full z-20 bg-white relative rounded-[10px] overflow-hidden aspect-[1/0.85] xl:aspect-[1/0.8]"
                    />
                ) : (
                    <CoverImage
                        src={urlFor(isDetail ? data.image : data.logo).url()} // Replace with the actual path to your image
            ...
            <div className="px-6 lg:px-8 rounded-[40px] py-2 lg:py-6 mb-2 lg:mb-2 text-center md:text-left">
                <H3
                    klasse={`${partner ? "!text-sm xl:!text-xl" : ""} mb-4 !font-sans !font-bold lg:mb-2 ${
